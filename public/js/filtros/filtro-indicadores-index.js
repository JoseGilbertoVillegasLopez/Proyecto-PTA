...
function initIndicadoresBasicosIndexSearch() {
    ...
    const page = document.querySelector(
        '[data-page="indicadores-basicos-index"]',
    );
    if (!page) return;
    ...
    input.addEventListener("input", () => {
        ...
        rows.forEach((tr) => {
            const nombre =
                tr
                    .querySelector(".indicadores-basicos-nombre")
                    ?.textContent.toLowerCase() ?? "";
            const formula =
                tr
                    .querySelector(".indicadores-basicos-formula")
                    ?.textContent.toLowerCase() ?? "";
            const observaciones =
                tr
                    .querySelector(".indicadores-basicos-observaciones")
                    ?.textContent.toLowerCase() ?? "";
...
document.addEventListener(
    "turbo:frame-load",
    initIndicadoresBasicosIndexSearch,
);

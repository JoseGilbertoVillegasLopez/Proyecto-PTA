document.addEventListener('turbo:load', inicializarPuestoNewCombobox);
document.addEventListener('turbo:frame-load', inicializarPuestoNewCombobox);
document.addEventListener('turbo:load', inicializarPuestoEditCombobox);
document.addEventListener('turbo:frame-load', inicializarPuestoEditCombobox);

function inicializarPuestoNewCombobox() {
    const contenedor = document.querySelector('.puesto-new-page-card');
    ...
    contenedor
        .querySelectorAll('select[id$="_supervisorDirecto"]')
        .forEach((select) => crearComboboxDesdeSelect(select));
}

function inicializarPuestoEditCombobox() {
    const contenedor = document.querySelector('.puesto-edit-page-card');
...
function crearComboboxDesdeSelect(select) {
    ...
    wrapper.className = 'puesto-select-combobox';
    ...
    input.type = 'text';
    input.className = `${select.className} puesto-select-combobox__input`.trim();
    input.autocomplete = 'off';
    input.placeholder = obtenerPlaceholder(select);
    wrapper.appendChild(input);

    const lista = document.createElement('ul');
    lista.className = 'puesto-select-combobox__list';
    lista.hidden = true;
    ...
    function renderizarLista(filtro) {
        ...
        if (filtradas.length === 0) {
            ...
            vacio.className = 'puesto-select-combobox__empty';
            vacio.textContent = 'Sin resultados';
            lista.appendChild(vacio);
        } else {
            filtradas.forEach((opcion) => {
                const item = document.createElement('li');
                item.className = 'puesto-select-combobox__item';
...
function crearToggleDesdeSelect(select) {
    ...
    const wrapper = document.createElement('div');
    wrapper.className = 'puesto-activo-toggle';
    select.insertAdjacentElement('beforebegin', wrapper);
    wrapper.appendChild(select);
    select.hidden = true;
    ...
    Array.from(select.options).forEach((opcion) => {
        ...
        boton.type = 'button';
        boton.className = 'puesto-activo-toggle__btn';

        if (opcion.selected) {
            boton.classList.add('puesto-activo-toggle__btn--active');
        }
        ...
        boton.addEventListener('click', () => {
            ...
            wrapper.querySelectorAll('.puesto-activo-toggle__btn').forEach((otro) => {
                otro.classList.toggle('puesto-activo-toggle__btn--active', otro === boton);
            });
        });

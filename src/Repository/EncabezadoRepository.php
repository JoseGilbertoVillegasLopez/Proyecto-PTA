...
class EncabezadoRepository extends ServiceEntityRepository
{
    ...
    public function findVisiblePta(array $access, array $filters = []): array
    {
        ...
        if (
            ...
            elseif ($access['scope'] === 'JERARQUICO') {
                ...
                // PROPIO + MISMO PUESTO DEL RESPONSABLE
                $qb
                    ->andWhere('p.id = :puesto')
                    ->setParameter('puesto', $filters['puesto_id'] ?? 0);
            }
        }
        ...
        if (!empty($filters['puesto'])) {
            $qb
                ->andWhere('p.id = :puestoFiltro')
                ->setParameter('puestoFiltro', $filters['puesto']);
        }
    ...
    public function findAniosDisponibles(array $access, int $personalId, ?int $puestoId = null): array
    {
        $qb = $this->createQueryBuilder('e')
        ...
        elseif ($access['scope'] === 'JERARQUICO') {
            ...
            $qb
                ->andWhere('p.id = :puesto')
                ->setParameter('puesto', $puestoId ?? 0);
        }
